...
#[derive(Default)]
pub struct AudioTransparentContentModel {
    audio_phrasing_context_stack: Vec<bool>,
}

impl Rule for AudioTransparentContentModel {
    fn id(&self) -> &'static str {
        "html.audio.transparent_content_model"
    }
        ...
        match event {
            ParseEvent::StartTag {
                ...
                if ctx.name_is(name, "audio") {
                    ...
                    self.audio_phrasing_context_stack.push(is_phrasing_ctx);
                    return;
                }

                if self
                    .audio_phrasing_context_stack
                    .last()
                    .copied()
                    .unwrap_or_default()
                    && !is_phrasing_element(ctx, name)
                {
                    ...
                    out.push(Message::new(
                        "html.audio.transparent.disallowed_child_in_phrasing",
                        Severity::Error,
                        Category::Html,
                        format!("Element “{child}” not allowed as child of “audio” in this context. Note: The “audio” element has a transparent content model; its allowed content is inherited from its parent element."),
                        *span,
                    ));
                }
            }
            ParseEvent::EndTag { name, .. } => {
                if ctx.name_is(name, "audio") {
                    self.audio_phrasing_context_stack.pop();
                }
            }
            _ => {}
        }
    ...
    fn on_finish(&mut self, _ctx: &mut ValidationContext, _out: &mut dyn MessageSink) {
        self.audio_phrasing_context_stack.clear();
    }
}
...
mod tests {
    ...
    fn codes(html: &str) -> Vec<String> {
        ...
        let report = html_inspector_core::validate_events(
            src,
            RuleSet::new().push(AudioTransparentContentModel::default()),
            Config::default(),
        )
        .unwrap();

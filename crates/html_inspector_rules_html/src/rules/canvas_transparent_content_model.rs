...
#[derive(Default)]
pub struct CanvasTransparentContentModel {
    canvas_phrasing_context_stack: Vec<bool>,
}

impl Rule for CanvasTransparentContentModel {
    fn id(&self) -> &'static str {
        "html.canvas.transparent_content_model"
    }
    ...
    fn on_event(
        ...
        match event {
            ParseEvent::StartTag {
                ...
                if ctx.name_is(name, "canvas") {
                    if *self_closing {
                        return;
                    }
                    let parent = ctx.current_parent();
                    let is_phrasing_ctx = parent.is_some_and(|p| is_phrasing_element(ctx, p));
                    self.canvas_phrasing_context_stack.push(is_phrasing_ctx);
                    return;
                }

                if self
                    .canvas_phrasing_context_stack
                    .last()
                    .copied()
                    .unwrap_or_default()
                    && !is_phrasing_element(ctx, name)
                {
                    ...
                    out.push(Message::new(
                        "html.canvas.transparent.disallowed_child_in_phrasing",
                        Severity::Error,
                        Category::Html,
                        format!("Element “{child}” not allowed as child of “canvas” in this context. Note: The “canvas” element has a transparent content model; its allowed content is inherited from its parent element."),
                        *span,
                    ));
                }
            }
            ParseEvent::EndTag { name, .. } => {
                if ctx.name_is(name, "canvas") {
                    self.canvas_phrasing_context_stack.pop();
                }
            }
            _ => {}
        }
    ...
    fn on_finish(&mut self, _ctx: &mut ValidationContext, _out: &mut dyn MessageSink) {
        self.canvas_phrasing_context_stack.clear();
    }
}
...
mod tests {
    ...
    fn codes(html: &str) -> Vec<String> {
        ...
        let report = html_inspector_core::validate_events(
            src,
            RuleSet::new().push(CanvasTransparentContentModel::default()),
            Config::default(),
        )
        .unwrap();

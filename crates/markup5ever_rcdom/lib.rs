        ...
        S: Serializer,
    {
        ...
        match traversal_scope {
            ...
            ChildrenOnly(_) => {
                ...
                ops.extend(children.iter().cloned().map(SerializeOp::Open));
            }
        }
            ...
            match op {
                SerializeOp::Open(handle) => match handle.data {
                    NodeData::Element {
                        ...
                        for child in children.iter().rev().cloned() {
...
#[cfg(test)]
mod tests {
    use super::{append, get_parent_and_index, Node, NodeData, SerializableHandle};
    ...
    #[test]
    fn get_parent_and_index_returns_none_when_parentless() {
        let node = Node::new(NodeData::Document);
        assert!(get_parent_and_index(&node).is_none());
    }

    #[test]
    fn get_parent_and_index_finds_child_position() {
        let parent = Node::new(NodeData::Document);
        let child1 = Node::new(NodeData::Text {
            contents: RefCell::new(StrTendril::from_slice("1")),
        });
        let child2 = Node::new(NodeData::Text {
            contents: RefCell::new(StrTendril::from_slice("2")),
        });
        append(&parent, child1);
        append(&parent, child2.clone());

        let (got_parent, idx) = get_parent_and_index(&child2).unwrap();
        assert!(std::rc::Rc::ptr_eq(&got_parent, &parent));
        assert_eq!(idx, 1);
    }
    ...
    fn qname(local: &str) -> markup5ever::QualName {
    ...
    #[test]
    fn serializes_include_node_and_children_only_consistently() {
        let root = Node::new(NodeData::Element {
            name: qname("div"),
            attrs: RefCell::new(vec![
                markup5ever::Attribute {
                    name: qname("id"),
                    value: StrTendril::from_slice("x"),
                },
                markup5ever::Attribute {
                    name: qname("class"),
                    value: StrTendril::from_slice("y"),
                },
            ]),
        ...
        let text = Node::new(NodeData::Text {
            contents: RefCell::new(StrTendril::from_slice("text node")),
        });
        ...
        let mut ser = TestSerializer::default();
        SerializableHandle::from(root.clone())
            .serialize(&mut ser, TraversalScope::IncludeNode)
            .unwrap();
        assert_eq!(
            ser.out,
            "<div id=\"x\" class=\"y\">text node<span></span></div>"
        );

        let mut ser = TestSerializer::default();
        SerializableHandle::from(root)
            .serialize(&mut ser, TraversalScope::ChildrenOnly(None))
            .unwrap();
        assert_eq!(ser.out, "text node<span></span>");
    }
}

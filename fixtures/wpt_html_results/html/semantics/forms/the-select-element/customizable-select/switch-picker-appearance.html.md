# html/semantics/forms/the-select-element/customizable-select/switch-picker-appearance.html
...
{
  "format_version": 1,
  "file": "html/semantics/forms/the-select-element/customizable-select/switch-picker-appearance.html",
...
promise_test(async (t) => {
  ...
  assert_true(select2.matches(':open'),'picker should open when clicked');
  assert_equals(getComputedStyle(select2).appearance,'base-select');
  await clickOn(secondOption); // Choose an option
  assert_false(select2.matches(':open'));
...
```json
{
  ...
  "source_name": "html/semantics/forms/the-select-element/customizable-select/switch-picker-appearance.html"
}
```

# html/semantics/interactive-elements/the-dialog-element/toggle-events.html
...
{
  "format_version": 1,
  "file": "html/semantics/interactive-elements/the-dialog-element/toggle-events.html",
...
<script>
  ["show", "showModal"].forEach((methodName) => {
    ...
    promise_test(async (t) => {
      assert_true(document.body.contains(mydialog),'still in the document');
      assert_false(mydialog.open,'initially closed');
      const abortController = new AbortController();
      const signal = abortController.signal;
      t.add_cleanup(() => {
        ...
        abortController.abort();
        waitForTick(); // Note that cleanups can't await
      });
...
```json
{
  ...
  "source_name": "html/semantics/interactive-elements/the-dialog-element/toggle-events.html"
}
```
